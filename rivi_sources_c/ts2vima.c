...
.IDENTIFICATION
  program TS2VIMA			version 1.00	930817
...
.VERSIONS
  1.00		first version
  1.10          better catalog handling, center wavelength + velocity interval
...
float     *inima, *tempima, *resima, *xrow, *xint, *tcol, *data;
...
for (frmcnt = t_limits[0]; frmcnt <= t_limits[1]; frmcnt++) {
   ...
   if (start[0]>x_limits[0] || (start[0]+npix[0]*step[0])<x_limits[1]){
     sprintf (msg, " error: selected region out of wavelength range in %s \n",
	             frame);
       SCETER(1,msg);
   }
...
for (t = 1; t < tnpix[1]; t++) {
  tdiff = mjd[t] - mjd[t-1];
  if (tdiff == 0) {
   sprintf (msg, " Error: cat.no #%d and #%d have the same time", t+t_limits[0]-1, t+t_limits[0]);
   SCTPUT (msg);
   SCETER (-1,msg);
  }
  if (tdiff < 0) {
...
strcpy (cunit,"rel.flux        velocity        JD-2400000.5    ");
